...
function renderCard(){
  prevText.textContent = midiToName(current.prevMidi, { showOctave: settings.showOctaves });
  const upArrow = current.up ? "↑" : "↓";
  intervalText.textContent = `${current.intervalId} ${upArrow}`;
  targetText.textContent = midiToName(current.targetMidi, { showOctave: settings.showOctaves });
...
async function playNote(midi, ms){
  ...
  await eng.init();
  try { await eng.playNote(midi, ms); }
...
function readSettingsFromUI(){
  const pool = INTERVALS.map(i=>i.id).filter(id => document.getElementById(`pool_${id}`).checked);
  settings = {
    ...settings,
...
// ---------- Boot ----------
function initPoolChecks(){ buildPoolUI(); }
function init(){
  initPoolChecks();
  fillNoteDatalist();
  applySettingsToUI();
